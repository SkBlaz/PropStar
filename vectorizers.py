...
class conjunctVectorizer:
    ...
    def fit(self, witem_documents):
        ...
        for witem_order in range(self.max_atoms[0], self.max_atoms[1]+1):
            ...
            all_combinations_of_order = list(combinations(self.overall_counts.keys(), witem_order))
            for candidate_feature in tqdm.tqdm(all_combinations_of_order, total = len(list(all_combinations_of_order))):
                if self.selected_witems.qsize() == self.queue_maxsize:
                    logging.info("Max priority queue size achieved. Breaking ..")
                    break
                
                all_counts = [self.overall_counts[x] for x in candidate_feature]
    ...
    def transform(self, witem_documents):
        ...
        m = sps.csr_matrix((values, (query, key)), shape=(doc_count,self.max_features))
        return m
